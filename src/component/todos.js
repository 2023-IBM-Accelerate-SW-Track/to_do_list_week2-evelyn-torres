...
const Todos = ({ todos, deleteTodo }) => {
    const todoList = todos.length ? (
      todos.map((todo) => {
        const handleDelete = () => {
          deleteTodo(todo.id); // Calling deleteTodo function with the todo item's id as a parameter
        };
        return (
          <Grid key={todo.id}>
            <Card style={{ marginBottom: "10px" }}>
              ...
              <ListItemButton component="a" href="#simple-list">
                <Checkbox 
                  style={{padding:2}} color="primary" 
                  onClick={handleDelete} />
                <ListItemText primary={todo.content} secondary={todo.data}/>
              </ListItemButton> 
            </Card>
          </Grid>
        );
